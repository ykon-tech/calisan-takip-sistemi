...
class Kullanici(object):
    ...
    def kayitOl(self):
        ...
        #! Kullanıcı bilgileri kaydedilirken ayırt edici bilgilere sahip birden fazla kullanıcı kaydı yapılabiliyor yani aynı e-posta ve kullanıcı adıyla bir başkası da kayıt yapabiliyor.

        #? Kullanıcı bilgilerinin kayıt olma işleminde unikliği üzerine düşün.

        #TODO: Eğer uniklik problemini çözmek istersen çöz.
    ...
    #!Verileri alıyor ama text dosyasında güncelleme konusunda sorun yaşıyor!
    def _hesapGuncelleme(self,eski_kullanici_adi):
        ...
        while True:
            ...
            if ' ' in yeni_kullanici_adi:
                print("\n ❌ Kullanıcı adı boşluk içeremez. Lütfen tekrar girin.")
                continue

            
            #* Kullanıcının veritabanında daha önce aynı ad ile kayıtlı olup olmadığı kontrol edilebilir.

            break   #Tüm kontrollerden geçti.
        ...
        while True:
            ...
            if not epostaDogrula(self,yeni_eposta):
                print("\n❌ Geçersiz e-posta formatı. Lütfen doğru e-posta formatı girin.")
                continue
            ...
            with open(DOSYA_YOLU, "r", encoding="utf-8") as dosya:
                ...
                if guncelleme_yapildi:
                    with open(DOSYA_YOLU, 'w',encoding="utf-8") as dosya:
                        dosya.writelines(tum_satirlar) #Güncellenmiş listeyi yazar.

                    print(f"\n ✅ Hesap bilgileri başarıyla güncellendi.")

                #Oturum bilgilerinde yeni kullanıcı adı geldiyse bunu geri dönüp göstermeliyiz.
